...
namespace TESTICP
{
    ICPSVDRegistration::ICPSVDRegistration(
        const YAML::Node &node) : input_target_kdtree_(new pcl::KdTreeFLANN<PointType>())
    {
        // parse params:
    ...
    ICPSVDRegistration::ICPSVDRegistration(
        ...
        int max_iter) : input_target_kdtree_(new pcl::KdTreeFLANN<PointType>())
    {
        SetRegistrationParam(max_corr_dist, trans_eps, euc_fitness_eps, max_iter);
    }
    ...
    bool ICPSVDRegistration::setTargetCloud(const CLOUD_PTR &input_target)
    {
    ...
    bool ICPSVDRegistration::scanMatch(
        const CLOUD_PTR &input_source,       //  输入待配准点云
        const Eigen::Matrix4f &predict_pose, // 预测位姿
        CLOUD_PTR &result_cloud_ptr,         //  转换位姿后的点云
    ...
    )
    {
        ...
        { //  最大迭代次数
            ...
            CLOUD_PTR curr_input_source(new CLOUD());
